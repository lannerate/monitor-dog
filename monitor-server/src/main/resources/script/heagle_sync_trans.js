...
var myCoursor = db.monitor_monitor_log.find({
    className: "com.monitor.tools.ctrl.security.interceptor.monitorTradeInterceptor",
    message: { $regex: 'transEnd=(?!null).*?$' },
...
myCoursor.forEach(function(o) {
    ...
    if (result) {
        ...
        trans_flag = 1;
        db.monitor_trans.save({ "trans_jnls_no": trans_jnls_no, "online_jnls_no": online_jnls_no, "session_id": session_id, "oper_no": oper_no, "process_code": process_code, "trans_begin_time": trans_begin_time, "trans_end_time": trans_end_time, "trans_duration": trans_duration, "trans_flag": trans_flag, timeStamp: now.getTime() });
